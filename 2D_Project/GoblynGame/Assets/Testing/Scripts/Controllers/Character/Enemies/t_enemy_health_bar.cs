...
public class t_enemy_health_bar : MonoBehaviour {
	...
	void Start(){
		...
		if(enemy_health_object != null){
			background_image = enemy_health_object.transform.GetChild (0).GetComponent <Image> ();
			foreground_image = enemy_health_object.transform.GetChild (1).GetComponent <Image> ();
		}
	...
	IEnumerator Parent_Healthbar_To_UI_Group(){
		...
		enemy_health_object.transform.parent = t_ui_group_enemy_health_controller.enemy_health_group_controller.transform;
	}
